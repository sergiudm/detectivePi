...
server_password = config.get_param("server_password")
...
# setup path
if use_camera:
    cap = cv2.VideoCapture(0) if os.name == "nt" else cv2.VideoCapture(1)
...
# q --- quit the program
if detect_other:
    working_detect(mpPose, pose, mpDraw, cap, image_path=image_path, pin=LED_pin)
else:
